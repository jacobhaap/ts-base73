...
const base73 = {
  ...
  toUnit8Array(base73String) {
    ...
    for (let i = 0; i < base73String.length; i++) {
      ...
      if (bitValue === undefined) {
        throw new Error(
          `Invalid character encountered during decoding: ${currentChar}`
        );
      }
    ...
    while (
      trimmedByteArray.length > 0 &&
      trimmedByteArray[trimmedByteArray.length - 1] === 0
    ) {
  ...
  buffer: {
    from(buffer) {
      return base73.Unit8Array.from(new Uint8Array(buffer));
    },
  },

  toBuffer(base73String) {
    const byteArray = base73.toUnit8Array(base73String);
  ...
  hex: {
    from(hexString) {
      ...
      return base73.Unit8Array.from(bytes);
    },
  },
  ...
  string: {
    from(str) {
      ...
      return base73.Unit8Array.from(byteArray);
    },
  },

  toString(base73String) {
    const byteArray = base73.toUnit8Array(base73String);
  ...
  decimal: {
    from(decimal) {
      ...
      return base73.Unit8Array.from(new Uint8Array(byteArray.reverse()));
    },
  },

  toDecimal(base73String) {
    const byteArray = base73.toUnit8Array(base73String);
    return byteArray.reduce(
      (acc, byte, index) =>
        acc + byte * Math.pow(256, byteArray.length - index - 1),
      0
    );
  },
